...
- **localStorage est synchrone** — l'app a été migrée depuis `window.storage` (API Codex.ai) vers `localStorage`. Si tu vois des `await` sur du storage local, c'est un résidu à nettoyer.
...
> « Audite le fichier `src/App.jsx` pour les références restantes à `window.storage` (l'ancien backend Codex.ai qu'on a remplacé par localStorage). Pour chaque occurrence, donne le numéro de ligne et l'extrait. Ne modifie rien. Réponds en liste. »
